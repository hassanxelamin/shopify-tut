...
export const ProductPageContent = ({ product }) => {
  const url = product.images.edges[0].node.url;
  const alt = product.images.edges[0].node.altText;

  return (
    <div className="flex flex-col justify-center items-center 
                    space-y-8 md:flex-row md:items-start md:space-y-0 
                    md:space-x-4 lg:space-x-8 max-w-6xl w-11/12 mx-auto"
    >
      <div className="w-full max-w-md border bg-white rounded-2xl 
                      overflow-hideen shadow-lg md:w-1/2"
      >
